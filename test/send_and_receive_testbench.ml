...
module Tb_source = Hardcaml_step_testbench.Functional.Make (Source) (Source)
module Tb = Hardcaml_step_testbench.Functional.Make (I) (O)

let rec send_data ~first ~num_words _ (h : Tb_source.Handler.t) : unit =
  if num_words = 0
  then ()
  else if Random.bool ()
  then (
...
let rec recv_data data (output : Tb.O_data.t) (h : Tb.Handler.t) : Bits.t list =
