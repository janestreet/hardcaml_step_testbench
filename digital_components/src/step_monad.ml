...
let wait_for (event : _ Event.t) ~output =
  let rec loop () =
    (* We use [thunk] to delay checking [Event.value] until the last possible moment, when
       the computation is being evaluated. This can avoid an unnecessary [next_step]. *)
    thunk (fun () ->
...
let run_effectful_computation computation =
  Step_core.Computation.Monadic.Exec_effectful computation
;;
