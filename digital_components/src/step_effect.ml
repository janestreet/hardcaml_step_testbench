...
let current_input (handler : ('i, 'o) Handler.t) =
...
let next_step here o (handler : _ Handler.t) =
  let%tydi { aliased_many = result } =
    Step_core.Computation.Eff.perform
      handler
...
let thunk f (handler : _ Handler.t) = f () handler

let output_forever (type i o) output (handler : (i, o) Handler.t) =
  while true do
    ignore (next_step [%here] output handler : i)
  done
;;

let wait_for (event : _ Event.t) ~output (handler : _ Handler.t) =
...
let wait ~output ~until (handler : ('i, 'o) Handler.t) =
...
let delay output ~num_steps (handler : ('i, 'o) Handler.t) =
  if num_steps < 0
  then
    raise_s
    ...
    Component.create
      (module struct
        ...
        let update_state ?prune ~parent_period ~step_number t =
          ...
          Runner.update_state ?prune ~update_children_after_finish ~period ~step_number t
        ;;
...
let run_monadic_computation (handler : ('i, 'o) Handler.t) computation =
  let%tydi { aliased_many } =
    Step_core.Computation.Eff.perform
      handler
