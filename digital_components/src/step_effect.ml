...
let next_step (handler : _ Handler.t) here o =
  let%tydi { aliased_many = result } =
    Step_core.Computation.Eff.perform
      handler
...
let thunk (handler : _ Handler.t) f = f () handler

let output_forever (type i o) (handler : (i, o) Handler.t) output =
  while true do
    ignore (next_step handler [%here] output : i)
  done
;;

let wait_for (handler : _ Handler.t) (event : _ Event.t) ~output =
...
let wait (handler : ('i, 'o) Handler.t) ~output ~until =
...
let delay (handler : ('i, 'o) Handler.t) output ~num_steps =
  if num_steps < 0
  then
    raise_s
...
let create_component
  ...
  ~(start : (i, o) Handler.t -> i -> (a, o) Component_finished.t)
  ~(input : i Data.t)
  ~(output : o Data.t)
  ()
